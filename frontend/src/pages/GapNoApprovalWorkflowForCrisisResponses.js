...
/**
 * Gap/CFS Feature Page: No approval workflow for crisis responses.
 * Slug: no-approval-workflow-for-crisis-responses
 * Backend: /api/gap-no-approval-workflow-for-crisis-responses
 */
export default function NoApprovalWorkflowForCrisisResponsesPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-approval-workflow-for-crisis-responses', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No approval workflow for crisis responses.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No approval workflow for crisis responses.</p>
