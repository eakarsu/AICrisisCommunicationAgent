...
const FEATURE_KEY = process.env.FEATURE_CRISIS_DETECTION_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Predictive crisis detection.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICrisisCommunicationAgent - crisis-detection',
    },
    body: JSON.stringify({
...
router.post('/crisis-detection', auth, async (req, res) => {
  try {
    ...
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_CRISIS_DETECTION_KEY');
    const ai = await callLLM({ feature: 'crisis-detection', goal: 'Predictive crisis detection', input: payload });
    // No persistence wired.
    return res.json({
      ok: true,
      feature: 'crisis-detection',
      endpoint: '/api/ai/crisis-detection',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
...
router.get('/crisis-detection/health', (req, res) => {
  res.json({
    feature: 'crisis-detection',
    endpoint: '/api/ai/crisis-detection',
